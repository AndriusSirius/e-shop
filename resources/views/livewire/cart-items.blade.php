        ...
        <div class="flex-1 mt-4">
            <div class="flex my-4">
                <div class="w-3/4 bg-white px-4 py-4">
                    @if(count($cart) < 1) <h5>Jusų krepšelyje prekių nėra. Prašome pridėti prekes, kad galėtumėte jas valdyti. </h5>
                        @else
                        ...
                        <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                            ...
                            <span class="text-center w-1/5 font-semibold text-sm">{{($data->products->price)*($data->quantity)}} €</span>
                            @endforeach
                            @endforeach

                            <button wire:click="removeCart({{$data->products->id}})" class="font-semibold hover:text-red-500 text-gray-500 text-lg border-none">x</button>
