        ...
        <div class="flex-1 mt-4">
            <div class="flex my-4">
                <div class="w-3/4 bg-white px-4 py-4">
                    ...
                    @else
                        <div class="flex mb-5 border-b p-5                        <h3 class=" font-semibold
                            text-black-600 text-xs uppercase w-2/5">Prekė</h3>
                    ...
                    @foreach ($cart as $data)
                        <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
                            ...
                            <span class="text-center w-1/5 font-semibold text-sm">{{ $data->products->price }}
                                €</span>
                            @foreach ($products as $product)
                                @foreach ($product->discounts as $disc)

                                    <span
                                        class="text-center w-1/5 font-semibold text-sm">{{ $data->products->price * $data->quantity }}
                                        €</span>
                                @endforeach
                            @endforeach

                            <button wire:click="removeCart({{ $data->products->id }})"
                                class="font-semibold hover:text-red-500 text-gray-500 text-lg border-none">x</button>
                ...
                <div id="summary" class="px-8 py-10 bg-gray-100 sm:w-1/3 md:w-1/3 lg:w-1/4">
                    ...
                    <button
                        class="inline-block px-6 py-2 text-xs font-medium leading-6 text-center text-white uppercase transition bg-blue-700 rounded-full shadow ripple hover:shadow-lg hover:bg-blue-800 focus:outline-none">Patvirtinti</button>
                    <div class="border-t mt-8">
                        ...
                        <button
                            class="border-2 text-sm uppercase w-full text-center border-blue-700 rounded-full font-bold text-blue-700 px-4 py-3 transition duration-300 ease-in-out hover:bg-blue-500 hover:text-white">pereiti
                            prie apmokėjimo</button>
                    </div>
                </div>
