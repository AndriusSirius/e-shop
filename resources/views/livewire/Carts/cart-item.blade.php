<div class="flex items-center hover:bg-gray-100 px-4 py-4">
    <div class="flex w-2/5">
        ...
        <div class="flex flex-col justify-between lg:ml-4 flex-grow">
    ...
    <div class="flex justify-center w-1/5 ml-4">

        <button wire:click="minusQuantity">
            <svg class="fill-current text-gray-600 w-2" viewBox="0 0 448 512">
                <path
                    d="M416 208H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h384c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z"/>
            </svg>
        </button>

        <input class="border text-center w-16 m-2 lg:w-14 md:w-14" type="text" wire:model="quantity">
    ...
    @else
        <span class="text-center lg:pl-4 ml-8 lg:mr-4 lg:ml-0 w-1/5 font-semibold lg:text-sm md:text-sm text-xs">
            {{$cart_item->products->price}} €
        </span>
        <span class="text-center ml-8 lg:ml-2 w-1/5 font-semibold lg:text-sm md:text-sm text-xs">
            {{$cart_item->products->price*($cart_item->quantity)}} €
        </span>
    @endif

    <button wire:click="delete()"
            class="font-semibold ml-2 mr-0 lg:ml-2 hover:text-red-500 text-gray-500 text-lg border-none">x
    </button>
</div>
